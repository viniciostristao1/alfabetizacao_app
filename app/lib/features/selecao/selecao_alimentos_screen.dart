...
class SelecaoAlimentosScreen extends StatefulWidget {
  const SelecaoAlimentosScreen({super.key});

  @override
  State<SelecaoAlimentosScreen> createState() => _SelecaoAlimentosScreenState();
}

class _SelecaoAlimentosScreenState extends State<SelecaoAlimentosScreen> {
  final _todos = todosOsAlimentos();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(n == 0 ? 'Selecionar alimentos' : 'Selecionados: $n'),
        actions: [
          if (n > 0)
            TextButton(
...
class _LinhaAlimento extends StatelessWidget {
  const _LinhaAlimento({
    required this.texto,
